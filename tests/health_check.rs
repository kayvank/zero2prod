...
async fn health_check_works() {
    ...
    let response = client
        .get( &format!("{}/health_check", test_app.address)) //("http://127.0.0.1:3000/health_check")
        .send()
        .await
...
async fn subscribe_return_200_for_valid_form_data() {
    ...
    let saved = sqlx::query!("SELECT email, name FROM subscriptions",)
        ...
        .await
        .expect("failed to fetched subscription.");
...
async fn spawn_app() -> TestApp {
    let mut configuration = get_configuration().expect("Failed to get application settings.");
    configuration.database.database_name = Uuid::new_v4().to_string();

    let db_pool =
        configure_db(&configuration.database).await;// .expect("Failed to migrate");
